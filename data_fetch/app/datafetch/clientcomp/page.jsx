  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#F4F7FE] p-6">
      <div className="w-full max-w-sm bg-white rounded-4xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-zinc-100 p-8 flex flex-col items-center">
        <div
          className={`w-20 h-20 rounded-full flex items-center justify-center mb-4 transition-transform hover:scale-105 duration-300 ${isMale ? "bg-blue-500 shadow-blue-200" : "bg-pink-500 shadow-pink-200"} shadow-lg`}
        >
          <User className="w-10 h-10 text-white" strokeWidth={2.5} />
        </div>

        <h2 className="text-2xl font-extrabold text-black capitalize mb-2">
          {userData.name}
        </h2>
        <div
          className={`px-4 py-1 rounded-full text-xs font-bold flex items-center gap-2 ${isMale ? "bg-blue-50 text-blue-600" : "bg-pink-50 text-pink-600"}`}
        >
          <div
            className={`w-1.5 h-1.5 rounded-full ${isMale ? "bg-blue-500" : "bg-pink-500"} animate-pulse`}
          ></div>
          {userData.gender}
        </div>
        ...
        <div className="w-full mt-10 space-y-6">
          ...
          <div className="flex justify-between items-center py-4 border-y border-zinc-50">
            ...
            <span className="text-sm font-black text-black">
              {userData.count?.toLocaleString() || 0}
            </span>
          </div>
...
export default DaatFetchClient;
